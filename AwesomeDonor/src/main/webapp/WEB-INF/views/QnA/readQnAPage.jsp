	...
	<div class="row">
		...
		<c:if test="${!empty sessionData }">
				...
				<div style="width: 800px; height: 450px; margin: 25px 10px 10px 25px;">
					...
					<div id="commentBox">
						...
						<c:choose>
							...
							<br>
							&nbsp;&nbsp;관리자&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(답변일 : ${boardData.dto_QnA_Comment.qnA_Comment_writeDate })<br>
							&nbsp;&nbsp;${boardData.dto_QnA_Comment.qnA_Comment_content }<br>
								<c:if test="${!empty sessionData.session_Admin }">
									<a href="./updateQnACommentPage?qnA_idx=${boardData.dto_QnA_Comment.qnA_idx }" class="btn btn-outline-secondary" style="float: right; margin: 0px 10px 0px 0px;">답변수정</a>
									<a href="./deleteQnACommentProcess?qnA_idx=${boardData.dto_QnA.qnA_idx }" class="btn btn-outline-secondary" style="float: right; margin: 0px 10px 0px 0px;">답변삭제</a>
								</c:if>
							</c:when>
							
							<c:otherwise>
								<c:if test="${!empty sessionData.session_Admin }">
									<form action="./writeQnACommentProcess" method="post">
										<textarea rows="5" cols="80" name="qnA_Comment_content"></textarea>
										<input type = "hidden" name="qnA_idx" value= ${boardData.dto_QnA.qnA_idx } >
										<button type="submit" class="btn btn-outline-secondary">답변달기</button>
									</form>
								</c:if>
								<c:if test="${empty sessionData.session_Admin }" >
									<br>
									<p>&nbsp;&nbsp;댓글이 아직 달려있지 않습니다.</p>
								</c:if>
							</c:otherwise>
						</c:choose>
					</div><!-- 댓글읽기/쓰기 박스 종료.-->
			...
			<c:if test="${empty sessionData }">
				...
				<a href="./QnAPage" class="btn btn-outline-secondary" style="float: right; margin: 0px 10px 0px 0px;">목록으로</a>
				<a href="./loginPage_Donor" class="btn btn-outline-secondary" style="float: right; margin: 0px 10px 0px 0px;">로그인</a>
			</c:if>	
		</div>	<!-- col-8 종료. -->
	</div>	<!-- row 종료. -->
